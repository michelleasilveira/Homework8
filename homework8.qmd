...
hourly <- raw_hourly %>%
  ...
  # For readability, ensure some friendly names (already snake_case from clean_names)
  rename(
...
set.seed(2025)
cv10 <- vfold_cv(train, v = 10, strata = seasons)
...
base_recipe <- function(dat) {
  recipe(bike_count ~ ., data = dat) %>%
    ...
    step_date(date, features = "wday", label = TRUE) %>%
    step_mutate(day_type = factor(if_else(date_wday %in% c("Sat","Sun"),
                                          "weekend","weekday"))) %>%
    step_rm(date_wday) %>%
...
# Interactions: seasons × holiday, seasons × temp, temp × rainfall
rec2 <- base_recipe(train) %>%
  step_interact(terms = ~ starts_with("seasons_"):starts_with("holiday_") +
                      starts_with("seasons_"):temp +
                      temp:rainfall)

# Quadratic terms for *each* numeric predictor (orthogonal polynomials)
